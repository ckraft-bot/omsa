...
C = 100 -> Accuracy: 0.957186544342508 = 95.72%
C = 200 -> Accuracy: 0.960244648318043 = 96.02%
C = 300 -> Accuracy: 0.969418960244648 = 96.94%
C = 400 -> Accuracy: 0.972477064220184 = 97.25%
C = 500 -> Accuracy: 0.977064220183486 = 97.71%
C = 600 -> Accuracy: 0.978593272171254 = 97.86%
C = 700 -> Accuracy: 0.981651376146789 = 98.17%
C = 800 -> Accuracy: 0.981651376146789 = 98.17%
C = 900 -> Accuracy: 0.981651376146789 = 98.17%
C = 1000 -> Accuracy: 0.980122324159021 = 98.01%
...
Observations and interpretations: 
In the scaled knn model K = 36 has the highest accuracy of 86.73% while the unscaled one the k = 29 has the highest accuracy of 70.41%. It seems that the unscaled models at their best are far from the performance of the scaled models.

Console outputs for knn _scaled_:
[1] "Max accuracy for scaled KNN model:"
[1] 0.8673469

Console outputs for knn _unscaled_:
[1] "Max accuracy for unscaled KNN model:"
[1] 0.7040816
...
for (K_value in 1:50) {
  ...
  # Fitting KNN model to training dataset (source: https://www.rdocumentation.org/packages/class/versions/7.3-22/topics/knn)
  pred <- knn(train = train_scale, # matrix of training set cases
              test = test_scale, # matrix of test set cases
              cl = train_labels, # factor of true classifications of training set
              k = K_value, # number of neighbors considered
              prob = TRUE) # If this is true, the proportion of the votes for the winning class are returned as attribute 'prob'
...
print(accuracies)
print("Max accuracy for scaled KNN model:")
...
for (K_value in 1:50) {
  ...
  # Fitting KNN model to training dataset without scaling
  pred <- knn(train = train_features,  # Using unscaled training data
              test = test_features,    # Using unscaled test data
              cl = train_labels,       # True classifications of training set
              k = K_value,             # Number of neighbors considered
              prob = TRUE)             # Proportion of votes for the winning class
...
print("Accuracies for each KNN model with varying K:")
print(accuracies)
print("Max accuracy for unscaled KNN model:")
print(max(accuracies))
...
plot <- ggplot(accuracy_data, aes(x = K, y = Accuracy)) +
  geom_line(color = "blue", size = 1) +    
  geom_point(color = "red", size = 3) +    
  labs(title = "KNN Accuracy for Different K Values (Unscaled Data)",
       x = "K Value",
       y = "Accuracy") +
  scale_y_continuous(labels = scales::percent) +   
  theme_minimal()
